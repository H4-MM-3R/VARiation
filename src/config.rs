...
impl TryFrom<Vec<String>> for Operation {
    ...
    fn try_from(value: Vec<String>) -> Result<Self> {
        ...
        if term == "add" {
            if value.len() != 3 {
                let err = anyhow!("add operation expects 2 arguments but got {}", value.len() - 1);
                return Err(err);
            } 

            let mut drain = value.drain(1..=2);
            return Ok(Operation::Add(drain.next().expect("expect to exist"), drain.next().expect("expect to exist")));
        }
        if term == "rm" {
            if value.len() != 2 {
                let err = anyhow!("remove operation expects 1 arguments but got {}", value.len() - 1);
                return Err(err);
            } 
        ...
        if value.len() > 1 {
            let err = anyhow!("print operation expects 0 or 1 arguments but got {}", value.len());
            return Err(err)
        }
            let arg = value.pop().expect("expect to exist");
